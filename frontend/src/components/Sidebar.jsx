...
export default function Sidebar({ active, onNav, isDark }) {
  const [filterValues, setFilterValues] = useState({ 0: "All Countries", 1: "All Products", 2: "All Risk Levels", 3: "Last 30 days" });

  const bg = isDark ? "bg-[#080c17] border-[#1e2a3a]" : "bg-[#f8faff] border-[#dde3f0]";
  const labelColor = isDark ? "text-[#3a4a5a]" : "text-[#aab0c0]";
  const itemInactive = isDark ? "text-[#6a7a8a] hover:text-white hover:bg-[#1a2535]" : "text-[#7a8aa0] hover:text-[#0a0e1a] hover:bg-[#eef2ff]";
  const divider = isDark ? "bg-[#1e2a3a]" : "bg-[#dde3f0]";
  const selectStyle = isDark
    ? "bg-[#0f1825] border-[#1e2a3a] text-[#8a9ab0] hover:border-[#2e3a4a]"
    : "bg-white border-[#dde3f0] text-[#5a6a8a] hover:border-[#c0cde0]";
  ...
  return (
    <aside className={`fixed top-12 left-0 w-52 h-[calc(100vh-3rem)] border-r flex flex-col overflow-y-auto z-40 transition-colors duration-300 ${bg}`}>
      <div className="p-4">
        <p className={`text-[10px] tracking-[0.2em] font-mono mb-3 ${labelColor}`}>NAVIGATION</p>
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2 rounded text-xs transition-all duration-150
                  ${isActive ? "bg-[#00e5ff] text-[#0a0e1a] font-bold" : itemInactive}`}
              >
                ...
                {item.badge && (
                  <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${isActive ? "bg-[#0a0e1a] text-[#00e5ff]" : "bg-[#ff4444] text-white"}`}>
                    {item.badge}
                  </span>
                )}
              </button>
      ...
      <div className="p-4">
        <p className={`text-[10px] tracking-[0.2em] font-mono mb-3 ${labelColor}`}>FILTERS</p>
        <div className="space-y-2">
          {filters.map((f, i) => (
            <select
              ...
              className={`w-full border text-xs px-3 py-2 rounded appearance-none cursor-pointer transition-colors ${selectStyle}`}
      ...
      <div className="mt-auto p-4">
        <div className={`border rounded p-3 ${mlBox}`}>
          ...
          <p className={`text-[10px] leading-relaxed ${isDark ? "text-[#6a7a8a]" : "text-[#5a8a6a]"}`}>Risk prediction model active. Scoring 8 live deals.</p>
        </div>
      </div>
    </aside>
  );
